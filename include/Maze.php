...
class Maze
{
	...
	public function getWallType($x,$y,$count=1) {
		
		if (
				($y==0) or 
				($y==$this->y*2) or 
				( ($x==0) and ($count == 1) ) or
				( ($x==$this->x) and ($count == 1) )
			) {
			$wallType = '#';
		} else {
			$wallType = 'X';
        ...
        for ($i = 0; $i < $printRows; $i++) {
            if ($i % 2 == 0) {
                ...
                for ($j = 0; $j < $this->x; $j++) {
					...
					$wallType = $this->getWallType($j,$i,1);
					$textMaze .= $wallType;
                    
					if ($this->walls[$currWall++] == 1) {
						$wallType = $this->getWallType($j,$i,2);
						$textMaze .= $wallType;
                    } else {
                        $textMaze .= " ";
                    }
                ...
                for ($j = 0; $j < $this->x; $j++) {
					...
					$cellText = " ";
					if ( 
							(!empty($this->startMark)) and
							($i==$this->startY) and 
							($j==$this->startX)
						) {
							$cellText = $this->startMark;
					}
					if ( 
							(!empty($this->finishMark)) and
							($i==$this->finishY) and 
							($j==$this->finishX)
						) {
							$cellText = $this->finishMark;
					}
